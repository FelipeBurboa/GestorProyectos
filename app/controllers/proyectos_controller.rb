class ProyectosController < ApplicationController
  ...
  def index
    ...
    if params[:estado].present?
      @proyectos = Proyecto.where('estado = ?', params[:estado])
    else
      @proyectos = Proyecto.all
    end
  ...
  def create
    ...
    respond_to do |format|
      if @proyecto.save
        format.html { redirect_to proyectos_path, notice: "Proyecto fue creado" }
      else
  ...
  private
    ...
    def proyecto_params
      params.require(:proyecto).permit(:nombre, :descripcion, :fechacomienzo, :fechatermino, :estado)
    end
end
